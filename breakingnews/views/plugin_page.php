...
if(isset($_POST['settings_submit'])){
	if(!empty($_POST['settings_submit'])){
		...
		if ($area_result or $bg_color_result or $text_color_result ){
			...
			$message = "<div class='message-div failed-message alert alert-failed'><p>Something went wrong</p></div>";
		}
...
<div class="container">
	...
	<form action="" method="post" name="bn-settings-form">
		...
		<div class="form-group settings-form-group">
			...
			<input type="text" name="area_title" value="<?php if(get_option( 'bn_area_title')){ echo get_option( 'bn_area_title');} else { echo 'Breaking News'; } ?>" class="form-control">
		</div>
		<div class="form-group settings-form-group">
			<label>Background Color</label>
			<input type="text" name="bg_color" class="form-control color-field bg-color" value="<?php if(get_option( 'bn_bg_color')){ echo get_option( 'bn_bg_color');} ?>">
		</div>
		<div class="form-group settings-form-group">
			<label>Text Color</label>
			<input type="text" name="text_color" class="form-control color-field text-color" value="<?php if(get_option( 'bn_text_color')){ echo get_option( 'bn_text_color');} ?>">
		</div>
...
?>
  	...
  	if(get_post_meta( get_the_ID(), 'bn_custom_title', true) ): ?>
  		<a href='<?php echo get_edit_post_link(); ?>'> <?php echo get_post_meta( get_the_ID(), 'bn_custom_title', true); ?> </a>
  	<?php else: ?>
